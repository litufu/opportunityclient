...
export const GET_COMPANY_PRODUCTS = gql`
  query CompanyProducts($inputvalue: String!) {
    companyProducts(inputvalue: $inputvalue){
      id
      name
    }
  }
...
function GetProducts({ inputValue,getItemProps,highlightedIndex,selectedItem }) {
  const { loading, error, data } = useQuery(GET_COMPANY_PRODUCTS,{
  ...
  return data.companyProducts.map(({name: item}, index) => (
    <MenuItem
      key={item}
